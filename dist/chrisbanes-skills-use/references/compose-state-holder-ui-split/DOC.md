...
If that UI-local state grows into coordinated behavior with multiple related fields and operations, use [`compose-state-hoisting`](../compose-state-hoisting/DOC.md) to decide whether it should become a plain state holder class remembered in composition.
...
- Frame-rate or UI-local values that should not force whole-tree recomposition when they change: prefer provider lambdas and deferred reads per [`compose-state-deferred-reads`](../compose-state-deferred-reads/DOC.md).

## Side effects

[`compose-side-effects`](../compose-side-effects/DOC.md) covers effect APIs (`LaunchedEffect`, `DisposableEffect`, `SideEffect`), keys, cleanup, and `rememberUpdatedState`.
...
- [`compose-ui-testing-patterns`](../compose-ui-testing-patterns/DOC.md) — testing plain state-driven UI composables without the full app graph.
- [`compose-state-hoisting`](../compose-state-hoisting/DOC.md) — deciding where UI element state and UI logic should live, including plain state holder classes.
- [`kotlin-multiplatform-expect-actual`](../kotlin-multiplatform-expect-actual/DOC.md) — platform services, native views, and expect/interface boundaries when shared UI meets platform-specific leaves.
